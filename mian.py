from game_board import game_board_row, separation, banner
import random
...
def pick_position(occupied_position, occupied_position_computer):
    while True:
        try:
            print("Please pick your row number and columns number between 1 to 3.")
...
while game_continue:
    ...
    while game_is_on:
        ...
        counter += 1
        display_gameboard()
        game_is_on = check_win(win_conditions, occupied_position, "You")

        if counter == 9 and game_is_on == True:
            game_is_on = False
            print("Draw")
